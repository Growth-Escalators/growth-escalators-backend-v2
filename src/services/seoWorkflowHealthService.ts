...
export async function ensureSeoTables(): Promise<void> {
  const stmts = [
    ...
    `ALTER TABLE keyword_rankings ADD COLUMN IF NOT EXISTS client_domain TEXT`,
    ...
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS clickup_task_id TEXT`,
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS clickup_task_url TEXT`,
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS priority_score INTEGER DEFAULT 0`,
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS published_url TEXT`,
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS outcome TEXT`,
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS outcome_measured_at TIMESTAMP`,
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS keyword TEXT`,
    `ALTER TABLE seo_opportunities ADD COLUMN IF NOT EXISTS notes TEXT`,
    `CREATE INDEX IF NOT EXISTS seo_opportunities_priority_idx ON seo_opportunities(priority_score DESC)`,
    `ALTER TABLE seo_alerts_log ADD COLUMN IF NOT EXISTS client_domain TEXT`,
    `ALTER TABLE seo_alerts_log ADD COLUMN IF NOT EXISTS message TEXT`,
    `ALTER TABLE seo_alerts_log ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()`,
