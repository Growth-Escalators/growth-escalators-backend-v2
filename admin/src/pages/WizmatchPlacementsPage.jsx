...
const STAGE_COLORS = { submitted: '#3B82F6', interviewing: '#F59E0B', offered: '#8B5CF6', started: '#10B981', ended: '#6B7280', lost: '#EF4444' };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Placements Pipeline</h1>
      </div>
      <div className="flex gap-4 overflow-x-auto pb-4">
        {STAGES.map(stage => (
          <div key={stage} className="flex-shrink-0 w-64" onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDrop(e, stage)}>
            <div className="rounded-t-lg p-2 text-white text-sm font-medium" style={{ backgroundColor: STAGE_COLORS[stage] }}>
            ...
            <div className="bg-gray-50 rounded-b-lg min-h-[200px] p-2 space-y-2">
              {placements.filter(p => p.status === stage).map(p => (
                <div key={p.id} draggable onDragStart={() => setDraggedId(p.id)} className="bg-white rounded shadow p-2 cursor-move hover:shadow-md">
                  <div className="font-medium text-sm">{p.candidate_first} {p.candidate_last}</div>
                  <div className="text-xs text-gray-500">{p.company_name}</div>
                  <div className="text-xs text-gray-400">{p.job_title}</div>
                  {p.margin_hourly && <div className="text-xs font-medium text-green-600">${p.margin_hourly}/hr margin</div>}
                </div>
              ))}
            </div>
          </div>
