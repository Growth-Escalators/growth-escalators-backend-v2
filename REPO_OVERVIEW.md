...
### Performance
5. **40+ database indexes added** — Migration `0030_fold_orphaned_indexes.sql` (folded from two earlier migration files that predated `_journal.json` tracking and were never actually applied by the migrator) covers tasks, bookings, invoices, payments, user_permissions, password_reset_tokens, and composite indexes for tenant+status queries
